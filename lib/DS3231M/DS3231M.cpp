...
DS3231M_Class::DS3231M_Class(TwoWire& wire):_wire(wire) {}   ///< Unused class constructor
...
bool DS3231M_Class::begin(const uint32_t i2cSpeed) {
  ...
  */
  //_wire.begin(32,33);                                // Start I2C as master device
  _wire.setClock(i2cSpeed);                     // Set I2C clock speed
  _wire.beginTransmission(DS3231M_ADDRESS);     // Address the DS3231M
  uint8_t errorCode = _wire.endTransmission();  // See if there's a device present
...
bool DS3231M_Class::init() {
 // _wire.begin(32,33,100000);                                // Start I2C as master device                   // Set I2C clock speed
  _wire.beginTransmission(DS3231M_ADDRESS);     // Address the DS3231M
  uint8_t errorCode = _wire.endTransmission();  // See if there's a device present
  if (errorCode == 0)                          // If we have a DS3231M
  {
...
uint8_t DS3231M_Class::readByte(const uint8_t addr) {
  ...
  */
  _wire.beginTransmission(DS3231M_ADDRESS);        // Address the I2C device
  _wire.write(addr);                               // Send the register address to read
  _TransmissionStatus = _wire.endTransmission();   // Close transmission
  _wire.requestFrom(DS3231M_ADDRESS, (uint8_t)1);  // Request 1 byte of data
  return _wire.read();                             // read it and return it
}  // of method readByte()
void DS3231M_Class::writeByte(const uint8_t addr, const uint8_t data) {
  ...
  */
  _wire.beginTransmission(DS3231M_ADDRESS);       // Address the I2C device
  _wire.write(addr);                              // Send the register address to write
  _wire.write(data);                              // Send the data to write
  _TransmissionStatus = _wire.endTransmission();  // Close transmission
}  // of method writeByte()
...
DateTime DS3231M_Class::now() {
  ...
  */
 _wire.beginTransmission(DS3231M_ADDRESS);        // Address the I2C device
  _wire.write(DS3231M_RTCSEC);                     // Start at specified register
  _TransmissionStatus = _wire.endTransmission();   // Close transmission
  _wire.requestFrom(DS3231M_ADDRESS, (uint8_t)7);  // Request 7 bytes of data
  if (_wire.available() == 7)                      // Wait until the data is ready
  {
    _ss = bcd2int(_wire.read() & 0x7F);         // Mask high bit in seconds
    _mm = bcd2int(_wire.read() & 0x7F);         // Mask high bit in minutes
    _hh = bcd2int(_wire.read() & 0x3F);         // Mask 2 high bits in hours and clamp to 0-23
    _wire.read();                               // Read and ignore Day-Of-Week register
    _d = bcd2int(_wire.read() & 0x3F);          // Mask 2 high bits for day of month
    _m = bcd2int(_wire.read() & 0x1F);          // Mask 3 high bits for Month
    _y = bcd2int(_wire.read()) + 2000;          // Add 2000 to internal year
