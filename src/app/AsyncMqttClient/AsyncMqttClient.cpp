...
*/
AsyncMqttClient& AsyncMqttClient::onConnect(mbed::Callback<void(bool)> callback) {
  _onConnectUserCallbacks.push_back(callback);
  return *this;
}

AsyncMqttClient& AsyncMqttClient::onDisconnect(mbed::Callback<void(AsyncMqttClientDisconnectReason)> callback) {
  _onDisconnectUserCallbacks.push_back(callback);
  return *this;
}

AsyncMqttClient& AsyncMqttClient::onSubscribe(mbed::Callback<void(uint16_t, uint8_t)> callback) {
  _onSubscribeUserCallbacks.push_back(callback);
  return *this;
}

AsyncMqttClient& AsyncMqttClient::onUnsubscribe(mbed::Callback<void(uint16_t packetId)> callback) {
  _onUnsubscribeUserCallbacks.push_back(callback);
  return *this;
}
...
uint16_t AsyncMqttClient::publish(const std::string &topic, const std::string &payload, uint8_t qos, bool retain)
{
 return  publish(topic.c_str(),qos,retain,payload.c_str(),payload.length(),true,0);
}
 
uint16_t AsyncMqttClient::publish(const std::string &topic, const char *payload, size_t payload_length, uint8_t qos,bool retain)
{
