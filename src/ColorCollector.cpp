...
void ColorCollector::run_task_collection()
{
    ...
    while(true){
        ...
        if (evt.status == osEventMail) {
            ...
            rgb.R.u16bit   = abs(product_stlb::api::get_rgb_properties().r_offset-_rgb_reg.R.u16bit);
            rgb.G.u16bit   = abs(product_stlb::api::get_rgb_properties().g_offset-_rgb_reg.G.u16bit);
            rgb.B.u16bit   = abs(product_stlb::api::get_rgb_properties().b_offset-_rgb_reg.B.u16bit);
            ...
            switch (mail->eventType)
            {
                ...
                {
                    doc["SentFrom"] = platformio::api::get_device_info().BoardID;
                    doc["r_offset"] = product_stlb::api::get_rgb_properties().r_offset;
                    doc["g_offset"] = product_stlb::api::get_rgb_properties().g_offset;
                    doc["b_offset"] = product_stlb::api::get_rgb_properties().b_offset;
                    TracePrinter::printTrace(doc["TowerColor"].as<String>());
                    if(lastColor!=doc["TowerColor"].as<String>()){
                        lastColor = doc["TowerColor"].as<String>();
                        invokeCallbackMqttPublish("TowerColor/"+platformio::api::get_device_info().BoardID,doc.as<String>());
                    }
                }
                break;
                case  MeasEventType::EventServerMeasure:
                {
                    ...
                    doc["g_offset"] = product_stlb::api::get_rgb_properties().g_offset;
                    doc["b_offset"] = product_stlb::api::get_rgb_properties().b_offset;
                    invokeCallbackMqttPublish("TowerColorMeasure/"+platformio::api::get_device_info().BoardID,doc.as<String>());
                }
                break;
                case  MeasEventType::EventWebAppOffset:
                {   
                    doc["box_mac_id"] = platformio::api::get_web_properties().ap_ssid;
                    int32_t red_diff = (_rgb_reg.R.u16bit-product_stlb::api::get_rgb_properties().r_offset);
                    int32_t green_diff = (_rgb_reg.G.u16bit-product_stlb::api::get_rgb_properties().g_offset);
                    int32_t blue_diff = (_rgb_reg.B.u16bit-product_stlb::api::get_rgb_properties().b_offset);
                    doc["r_diff"] =  abs(red_diff);
                    doc["g_diff"] =  abs(green_diff);
                    doc["b_diff"] =  abs(blue_diff);

                    product_stlb::api::get_rgb_properties().r_offset = _rgb_reg.R.u16bit;
                    product_stlb::api::get_rgb_properties().g_offset = _rgb_reg.G.u16bit;
                    product_stlb::api::get_rgb_properties().b_offset = _rgb_reg.B.u16bit;
                    ...
                    text=String("{\"r_offset\":")+String(product_stlb::api::get_rgb_properties().r_offset,DEC)
                                 +String(",\"g_offset\":")+String(product_stlb::api::get_rgb_properties().g_offset,DEC)
                                 +String(",\"b_offset\":")+String(product_stlb::api::get_rgb_properties().b_offset,DEC)+String("}");
                    
                    if(FatHelper.writeFile(FFat,product_stlb::api::get_rgb_properties().path.c_str(),text)){
                        doc["file_write"]=true;
                    }else{
