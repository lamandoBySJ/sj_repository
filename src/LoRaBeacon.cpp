...
void LoRaBeacon::startup()
{
    ...
    _topicCommand = Platform::getDeviceInfo()->Family+ String("/command/request/BCN");
    ...
    while(true){
        ...
        if (evt.status == osEventMail) {
            ...
            if(mail->receiver == platform_debug::Platform::getDeviceInfo()->BoardID){
                ...
                _mqttNetwork.publish("k49a/send_rssi/"+Platform::getDeviceInfo()->BoardID,endNodeDoc.as<String>());
                endNodeDoc.clear();
                platform_debug::TracePrinter::printTrace("[BCN]lora:Rx:PUB:send_rssi:beaconID:"+
                    platform_debug::Platform::getDeviceInfo()->BoardID+
                    ",tagID:"+mail->sender+
                    ",rssi:"+mail->rssi);
            }else {
