...
void LoRaBeacon::startup()
{
    ...
    _topicCommand = Platform::deviceInfo.Family+ String("/command/request/BCN");
    ...
    while(true){
        ...
        if (evt.status == osEventMail) {
            ...
            if(mail->receiver == platform_debug::Platform::deviceInfo.BoardID){
                ...
                _mqttNetwork.publish("k49a/send_rssi/"+Platform::deviceInfo.BoardID,endNodeDoc.as<String>());
                endNodeDoc.clear();
                platform_debug::TracePrinter::printTrace("[BCN]lora:Rx:PUB:send_rssi:beaconID:"+
                    platform_debug::Platform::deviceInfo.BoardID+
                    ",tagID:"+mail->sender+
                    ",rssi:"+mail->rssi);
            }else {
