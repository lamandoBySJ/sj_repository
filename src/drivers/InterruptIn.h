 ...
 * @endcode
 */
class InterruptIn : public gpio_t ,private NonCopyable<InterruptIn> {
...
public:
    ...
    static void _irq_handler_thunk(void* id)
    {
      InterruptIn *handler =reinterpret_cast<InterruptIn*>(id);
        switch (handler->gpio_irq.event) {
            case IRQ_RISE:
                if (handler->_rise) {
                    handler->_rise();
                }
