...
{
    _topicCommandResponse = Platform::getDeviceInfo()->Family+ String("/command/response/GW");
    _topicCommandRequest = Platform::getDeviceInfo()->Family+ String("/command/request/GW");
    _topicTimeout = Platform::getDeviceInfo()->Family+String("/send_timeout/#");
                    ...
                    if(doc.containsKey("beacons")){
                        ...
                        _IPSProtocol.mode       = doc["mode"].as<String>();
                            ...
                            // if(doc.containsKey("cmd")&&doc.containsKey("tagID")){
                                ...
                                if( cmd== "learn"){
                                    _mode="learn"; 
                                    _loRaNetwork.sendMessage(tagID,platform_debug::Platform::getDeviceInfo()->BoardID,"{\"cmd\":\"learn\"}"); 
                                }else if( cmd == "track"){
                                    _mode="track";     
                                    _loRaNetwork.sendMessage(tagID,platform_debug::Platform::getDeviceInfo()->BoardID,"{\"cmd\":\"track\"}"); 
                                }else if( cmd == "ON"){
                                    _loRaNetwork.sendMessage(tagID,platform_debug::Platform::getDeviceInfo()->BoardID,"{\"cmd\":\"ON\"}");
                                }else if( cmd == "OFF"){
                                    _loRaNetwork.sendMessage(tagID,platform_debug::Platform::getDeviceInfo()->BoardID,"{\"cmd\":\"OFF\"}");
                                }
                           // }    
                    }  
                    _mqttNetwork.publish(_topicCommandResponse,"{\""+Platform::getDeviceInfo()->BoardID+"\":\"OK\"}");         
                }else if(_topicSplit.size()== 3){
                     _loRaNetwork.sendMessage(_topicSplit[2],platform_debug::Platform::getDeviceInfo()->BoardID,"{\"cmd\":\""+_IPSProtocol.mode+"\"}");
...
void LoRaGateway::run_lora_service()
{
    ...
    while(true){
        ...
        if (evt.status == osEventMail) {
            ...
            if(mail->receiver == platform_debug::Platform::getDeviceInfo()->BoardID){
